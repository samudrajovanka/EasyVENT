...
const fetchData = {
  ...
  getEventsByUsername: (username, page = 1) => {
    ...
    return {
      page,
      maxPage: Math.ceil(eventsFilter.length / maxList),
      data: {
        events: eventList,
      },
    };
  ...
  register: ({
    ...
    const newUserFollowers = {
      id: usersFollowers.length + 1,
      username: username.toLowerCase(),
      followers: [],
    };

    usersFollowers.push(newUserFollowers);

    const newUserFollowing = {
      id: usersFollowing.length + 1,
      username: username.toLowerCase(),
      following: [],
    };

    usersFollowing.push(newUserFollowing);
